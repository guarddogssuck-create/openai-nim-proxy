...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    // Only attach chat_template_kwargs for models that actually support it
    const supportsThinking = ENABLE_THINKING_MODE && THINKING_CAPABLE_MODELS.includes(nimModel);

    // Transform OpenAI request to NIM format.
    // NOTE: axios has no concept of "extra_body" (that's an OpenAI-SDK-only
    // convenience). Extra fields must be merged directly into the top-level
    // request object, or NIM's stricter backends will reject the whole
    // request with a 400.
    const nimRequest = {
      model: nimModel,
      messages: messages,
      temperature: temperature || 0.85,
      max_tokens: max_tokens || 32024,
      stream: stream || false,
